...
ZEND_API HashPosition ZEND_FASTCALL zend_hash_iterator_pos(uint32_t idx, HashTable *ht)
{
	...
	{
		if (EXPECTED(iter->ht) && EXPECTED(iter->ht != HT_POISONED_PTR) && EXPECTED(!HT_ITERATORS_OVERFLOW(iter->ht)))
		{
...
ZEND_API HashPosition ZEND_FASTCALL zend_hash_iterator_pos_ex(uint32_t idx, zval *array)
{
	...
	if (UNEXPECTED(iter->ht != ht))
	{
		if (EXPECTED(iter->ht) && EXPECTED(iter->ht != HT_POISONED_PTR) && EXPECTED(!HT_ITERATORS_OVERFLOW(ht)))
		{
			HT_DEC_ITERATORS_COUNT(iter->ht);
		}
...
static zend_always_inline Bucket *zend_hash_str_find_bucket(const HashTable *ht, const char *str, size_t len, zend_ulong h)
{
	...
	while (idx != HT_INVALID_IDX)
	{
		...
		p = HT_HASH_TO_BUCKET_EX(arData, idx);
		if ((p->h == h) && p->key && zend_string_equals_cstr(p->key, str, len))
		{
			return p;
		}
		idx = Z_NEXT(p->val);
...
static zend_always_inline zval *_zend_hash_index_add_or_update_i(HashTable *ht, zend_ulong h, zval *pData, uint32_t flag)
{
	...
	if (HT_IS_PACKED(ht))
	{
		// 判断 是否修改和添加的位置  小于 ht->nNumUsed
		if ((flag & (HASH_ADD_NEW | HASH_ADD_NEXT)) != (HASH_ADD_NEW | HASH_ADD_NEXT) && h < ht->nNumUsed)
		{
			zv = ht->arPacked + h;
		...
		add_to_packed:
			...
			if ((flag & (HASH_ADD_NEW | HASH_ADD_NEXT)) != (HASH_ADD_NEW | HASH_ADD_NEXT))
			{
				if (h > ht->nNumUsed)
				{
					//  ht->nNumUsed < h < ht->nTableSize, 说明插入的位置和最后一个元素之间索引不连续
					zval *q = ht->arPacked + ht->nNumUsed;
					// 将 [ht->nNumUsed, h) 间的元素置为未定义
					while (q != zv)
					{
						ZVAL_UNDEF(q);
						q++;
		...
		else
		{
			...
			// ？？？ 这个情况是什么情况， 是向packed array中添加了一个数值健，hash值很大
		convert_to_hash:
			zend_hash_packed_to_hash(ht);
		}
	}
...
ZEND_API HashTable *ZEND_FASTCALL zend_array_dup(HashTable *source)
{
	...
	else if (HT_IS_PACKED(source))
	{
		...
		target->nInternalPointer =
			(source->nInternalPointer < source->nNumUsed) ? source->nInternalPointer : 0;
...
ZEND_API void ZEND_FASTCALL zend_hash_sort_ex(HashTable *ht, sort_func_t sort, bucket_compare_func_t compar, bool renumber)
{
	...
	sort((void *)ht->arData, ht->nNumUsed, sizeof(Bucket), (compare_func_t)compar,
		 (swap_func_t)(renumber ? zend_hash_bucket_renum_swap : (HT_IS_PACKED(ht) ? zend_hash_bucket_packed_swap : zend_hash_bucket_swap)));
