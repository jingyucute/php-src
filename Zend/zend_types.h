...
typedef union _zend_value
{
	...
	zend_refcounted *counted; // 获取不同类型结构的gc头部指针
	zend_string *str;		  // 字符串类型指针
	zend_array *arr;		  // 数组类型指针
	zend_object *obj;		  // 对象类型指针
	zend_resource *res;		  // 资源类型指针
	zend_reference *ref;	  // 引用类型指针， 比如 $a = 1; $b = &$a;
	zend_ast_ref *ast;		  // ast 指针，线程安装使用的，内核使用
	zval *zv;				  // 指向另外一个zval的指针 内核使用
	void *ptr;				  // 指针，通用类型， 内核使用
	zend_class_entry *ce;	  // 类， 内核使用
	zend_function *func;	  // 函数， 内核使用
	struct
	{
		uint32_t w1;
		uint32_t w2;
...
struct _zval_struct
{
	...
	union
	{
		...
		{
			ZEND_ENDIAN_LOHI_3(
				uint8_t type, /* active type */ // 变量类型
				uint8_t type_flags,				// 可以用于变量的分类
				union {
	...
	union
	{
		uint32_t next;			 // 这里是用作hash地址冲突的碰撞链	 /* hash collision chain */
...
struct _zend_string
{
	zend_refcounted_h gc; // 占用8个字节， 用户gc记录和字符串类型的记录
	zend_ulong h;		  // 占用8个字节， 用户记录字符串的hash值
	size_t len;			  // 占用8个字节， 字符串长度
	char val[1];		  // 占用一个字节，字符串的值存储位置
};

typedef struct _Bucket
{
	zval val;		  // 数组的值
	zend_ulong h;	  // key的hash值，如果是数值的就是数值  /* hash value (or numeric index)   */
	zend_string *key; // 数组key的指针，当key是字符串的时候，就是指针，当key是数字的时候就是null /* string key or NULL for numerics */
} Bucket;
...
struct _zend_array
{
	zend_refcounted_h gc; // 占用8个字节， 引用计数和字符串类型
	union
	{
		struct
		{
			ZEND_ENDIAN_LOHI_4(
				uint8_t flags, // 8位无符号字符，最大值255, 用户标记hashtable,
				uint8_t _unused,
				uint8_t nIteratorsCount, // 迭代器计数， 迭代器包括正在遍历的hashtable和游标信息
				uint8_t _unused2)
		} v;
		uint32_t flags;
	} u;				 // 这里的u是一个联合体，占用4个字节， 可以存储一个uint32_t 的flags， 也可以存储由4个unsigned char组成的结构体v
	uint32_t nTableMask; // 哈希计算掩码， 等于nTableSize的负值(nTableMask = -nTableSize)
	union
	{
	...
	};							// 三选一，其中一个赋值后，其他会变成为定义
	uint32_t nNumUsed;			// 指所有已使用bucket的数量，包括有效bucket和无效bucket的数量
	uint32_t nNumOfElements;	// 有效bucket的数量。该值总是小于或等于nNumUsed
	uint32_t nTableSize;		// HashTable的大小 表示arData指向的bucket数组的大小，即所有bucket的数量
	uint32_t nInternalPointer;	// 全局默认游标
	zend_long nNextFreeElement; // 下一个插入的元素的key的下标
	dtor_func_t pDestructor;	// 指向一个函数， pDest是zval结构指针二级指针
};
...
#define HT_INVALID_IDX ((uint32_t)-1) // 哈希区无效索引-1

#define HT_MIN_MASK ((uint32_t)-2) // 哈希表最小的hash区大小 (int)-2 = 2
#define HT_MIN_SIZE 8			   // 哈希表最小的大小 8
 ...
 */
// 这里的SIZEOF_SIZE_T，应该是为了是配操作系统的位数
#if SIZEOF_SIZE_T == 4
...
#define HT_HASH(ht, idx) \
	HT_HASH_EX((ht)->arHash, idx) // 这里不清楚，arHash的作用， 获取偏移位置？

#define HT_SIZE_TO_MASK(nTableSize) \
	((uint32_t)(-((nTableSize) + (nTableSize)))) // 之前负载因子为1,现在为0.5 nTableMask == - (nTableSize + nTableSize)
#define HT_HASH_SIZE(nTableMask) \
	(((size_t) - (uint32_t)(nTableMask)) * sizeof(uint32_t)) // 获取hash区的内存大小
#define HT_DATA_SIZE(nTableSize) \
	((size_t)(nTableSize) * sizeof(Bucket)) // 获取数据区的大小
#define HT_SIZE_EX(nTableSize, nTableMask) \
	(HT_DATA_SIZE((nTableSize)) + HT_HASH_SIZE((nTableMask))) // 获取整个哈希表的内存大小
#define HT_SIZE(ht) \
	HT_SIZE_EX((ht)->nTableSize, (ht)->nTableMask) // 获取整个哈希表的内存大小
#define HT_USED_SIZE(ht) \
	(HT_HASH_SIZE((ht)->nTableMask) + ((size_t)(ht)->nNumUsed * sizeof(Bucket))) // hash array 获取使用部分的哈希表的大小
#define HT_PACKED_DATA_SIZE(nTableSize) \
	((size_t)(nTableSize) * sizeof(zval)) // 获取packed array 数据区大小
#define HT_PACKED_SIZE_EX(nTableSize, nTableMask) \
	(HT_PACKED_DATA_SIZE((nTableSize)) + HT_HASH_SIZE((nTableMask))) // packed array 获取内存大小
#define HT_PACKED_SIZE(ht) \
	HT_PACKED_SIZE_EX((ht)->nTableSize, (ht)->nTableMask) // packed array 获取内存大小
#define HT_PACKED_USED_SIZE(ht) \
	(HT_HASH_SIZE((ht)->nTableMask) + ((size_t)(ht)->nNumUsed * sizeof(zval))) // packed array 获取使用zval部分大小
#if defined(__AVX2__)
